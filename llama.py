""" To run the file and get the model running, simply run the command/file ` ./run.sh ` """
...
# check the model type provided | if ggml or gptq or full precision
...
# check if folder containing the model exists and create if False
def create_folder_if_not_exists(folder_path):
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)

# initialises the gpu and tokenizer 
...
def init_auto_model_and_tokenizer(model_name, model_type, file_name=None):
  ...
  if Model_Type.ggml == model_type:
    ...
    # download model from huggingface (model size is 4.08GB) if not found 
...
def run_ui(model, tokenizer, is_chat_model, model_type):
  """ responsible for the UI for gradio and the chat interface between user and assistant """
...
# check if command line arguments have been provided ( run.sh file )
